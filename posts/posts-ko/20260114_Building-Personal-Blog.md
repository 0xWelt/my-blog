...
### 아키텍처 진화(2026-08 업데이트)

2026-08에 배포 아키텍처를 통합 최적화했습니다: 다국어를 2개에서 8개(중국어, 영어, 일본어, 한국어, 아랍어, 프랑스어, 독일어, 이탈리아어)로 확장하고, 배포 방식을 「여러 Pages + Worker 프록시」에서 「단일 Pages 프로젝트」로 단순화했습니다:

1. **아키텍처 통합**: blog-proxy Worker 프록시와 언어별로 분리된 8개의 Pages 프로젝트를 폐기하고, 단일 Pages 프로젝트(blog)로 변경하여 모든 언어를 하나의 프로젝트에 통합했습니다
   - 메인 도메인 `blog.0xwelt.com` → CNAME → `blog-5up.pages.dev`
   - 중국어: `/`(접두사 없음); 기타 언어: `/{lang}/`(접두사 포함)

2. **통합 빌드(build-all)**: `scripts/build-all.sh`가 언어별로 Hexo 빌드를 실행(각 언어는 여전히 독립적인 설정 파일 사용)하고, 산출물을 `public-all/`에 병합합니다(중국어는 루트 디렉터리, 기타 언어는 `/<lang>/` 하위 디렉터리)

3. **배포 방식 단순화**: GitHub main에 push하면 Pages 자동 빌드 배포가 트리거되며, 프록시 계층이 없고 수동 배포가 필요하지 않습니다

언어 전환 로직은 변경되지 않으며, 여전히 프론트엔드 JavaScript를 통해 `blog.0xwelt.com` 통합 도메인에서 완료됩니다.
